...
val=livespell.str.HTMLDec(val);val=val.replace(/<br[ ]*[\/]?>/gi,"\n")
oField[t]=val;}}
...
for(var i=0;i<fieldIds.length;i++){var id=fieldIds[i];var oField=document.getElementById(id);var t=this.FieldType(id);var val;if(t==="iframe"){var oDoc=livespell.getIframeDocumentBasic(oField);val=oDoc.body.innerHTML;}else{if(livespell.test.IE()){val=livespell.str.HTMLDec(livespell.str.convertToText(oField));}else{val=oField[t];}
val=livespell.str.HTMLEncSafeWhite(val);}
docs[i]=val;}
return docs;}
...
var fn=function(){f["onDialogOpen"]()};setTimeout(fn,150)}else if(wm=="modalbox"&&typeof(Modalbox)!=="undefined"){var HTMLIframe="<iframe width='460' height='290' scrolling='no' marginwidth='0' marginheight='0' src='"+this.url()+"' frameborder='0' ></iframe>"
...
var fn=function(){f["onDialogOpen"]()};setTimeout(fn,189)}else if(wm=="modal"&&window.showModalDialog&&canmodal){var dheight=livespell.test.IE6()?333:290;window.showModalDialog(this.url(),window,"center:1;dialogheight:"+dheight+"px;dialogwidth:460px;resizable:0;scrollbars:0;scroll:0;location:0");}else if(wm=="modeless"&&window.showModelessDialog&&canmodal){var dheight=livespell.test.IE6()?333:290;window.showModelessDialog(this.url(),window,"center:1;dialogheight:"+dheight+"px;dialogwidth:460px;resizable:0;scrollbars:0;scroll:0;location:0");}else{window.open(this.url(),"spelldialog","width=460,height=290,scrollbars=no,resizable=no;centerscreen=yes;location=no;tolbar=no;menubar=no",false);}}
...
xhr.open("GET",posturl+"?"+params,false);xhr.send();livespell.ajax.debug("URL:"+posturl+"  <br/>  GET:"+params,true)}else{var e;livespell.ajax.renderIframe(posturl);params+="&script=true"
...
return false;},webkit:function(){return/webkit/.test(navigator.userAgent.toLowerCase())},Safari:function(){return/safari/.test(navigator.userAgent.toLowerCase())},FireFox:function(){return/firefox/.test(navigator.userAgent.toLowerCase())},BuggyAjaxInFireFox:function(){return/firefox\/3./.test(navigator.userAgent.toLowerCase())&&/windows/.test(navigator.userAgent.toLowerCase())},MAC:function(){if(navigator.platform&&(navigator.platform.toUpperCase().indexOf("MAC")>-1)){return true;}
...
strinput=strinput.replace((/(\<\/a[^>]*\>)/gi),"");return strinput.replace((/(\<a[^>]*\>)/gi),"");},stripNonSpaceTags:function(strinput,oinput){if(!strinput){return""}
...
strinput=strinput.replace((/<!--[\s\S.\n]*?-->/g),"");strinput=strinput.replace((/&lt;!--[\s\S.\n]*?--&gt;/g),"");return strinput;},HTMLEnc:function(s){s=(s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'));s=s.replace(/\n/g,"<br />");s=s.replace(/[ ][ ]/gi," &nbsp;");s=s.replace(/[ ][ ]/gi," &nbsp;");return s;},HTMLEncSafeWhite:function(s){s=(s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'));s=s.replace(/\n/g,"<br />");s=s.replace(/[ ][ ]/gi," &nbsp;");s=s.replace(/[ ][ ]/gi," &nbsp;");return s;},HTMLDec:function(s){s=s=s.replace(/\&nbsp\;/gi," ");s=(s.replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>'));return s;},HTMLDecAndStripFormatting:function(s){if(livespell.test.IE()){s=s.replace((/\n/gi),"");}else{s=s.replace((/\n/gi),"<BR>");s=s.replace((/\r/gi),"<BR>");}
...
if(e.nodeName.toLowerCase()!=="textarea"){return};livespell.insitu.init();if(document.getElementById(id+this._FIELDSUFFIX)){return livespell.insitu.proxyDOM(id);};var attr,stylesToCopy,i,styleval,memreadonly;var n=document.createElement("div");n.setAttribute("id",id+this._FIELDSUFFIX);var t=E$(id);if(livespell.test.IE()){try{memreadonly=t.readOnly;t.readOnly=false}catch(e){}}
...
n.onpaste=function(){try{var value=window.clipboardData.getData("Text");if(livespell.test.IE()){value=value.replace(/\n/g,"\r\n")}
window.clipboardData.setData("Text",value);}catch(e){}
try{var bid=(n.id.replace(livespell.insitu._FIELDSUFFIX,''))
livespell.insitu.updateBase(bid);}catch(e){}}}
...
var me=e.srcElement?e.srcElement:this;if(me.autocheck){me.unChanged=false;var base_field_id=me.id.split(livespell.insitu._FIELDSUFFIX)[0];var ProviderId=Number(me.autocheckProvider);if(!livespell.test.IE()||me.innerHTML.indexOf("<!-")>-1){var a=me.innerHTMLs
