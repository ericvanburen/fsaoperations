...
<script runat="server">
    ...
    Protected Sub btnUploadForm_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        strSql = "p_PCAEmployeeUploadIRTForm"
        ...
        If strFileNamePath.Length > 0 Then
            ...
            'strSaveLocation = "D:\DCS\secure\IMF_ED\x49g\pca\employees\irt\" & strFileNameOnly
            strSaveLocation = lblUploadPath.Text & "pca\employees\irt\" & strFileNameOnly
            
            Filebox.PostedFile.SaveAs(strSaveLocation)
            cmd.Parameters.Add("@IRT_Attachment", SqlDbType.VarChar).Value = strFileNameOnly
        Else
            cmd.Parameters.Add("@IRT_Attachment", SqlDbType.VarChar).Value = DBNull.Value
        End If
        ...
        lblUpdateStatus.Text = "Your IRT form has been received. Please click the Close Window button to return to the Employee Details screen"
    ...
    Sub ResetAcceptFormValue()
        'Sometimes the PCAs need to resubmit a IRT form because a previous one was rejected or not approved by ED and there may be an old
        'value in the field IRT_Attachment_Accepted_By_ED so we need to set that back to it's original value = NULL
        ...
        cmd.CommandType = CommandType.StoredProcedure
        cmd.Parameters.AddWithValue("@ID", lblID.Text)
        cmd.Parameters.AddWithValue("@FormType", "IRT")
...
<head runat="server">
    <title>Upload Incidence Response Training (IRT) Form</title>
...
<body>
    <form id="form1" runat="server">
     <fieldset>
    <legend class="fieldsetLegend">Upload Incidence Response Training (IRT) Form</legend><br />    
    <div align="left">
    <table>
    <tr>
            <td class ="formLabel" width="50%" >Date Incidence Response Training (IRT) Completed:</td>
            <td width="50%" ><asp:Textbox ID="txtIRT_Date" runat="server"  /> (mm/dd/yyyy)<br />
            <asp:RequiredFieldValidator ID="RequiredFieldValidator1" runat="server" ErrorMessage="* This field is required * " CssClass="warningMessage" Display="Dynamic" ControlToValidate="txtIRT_Date" />
            <asp:CompareValidator ID="dateValidator" runat="server" Type="Date" Operator="DataTypeCheck" ControlToValidate="txtIRT_Date" ErrorMessage="* Please enter a valid date (mm/dd/yyyy) *" CssClass="warningMessage" Display="Dynamic" /></td>              
    </tr>        
    <tr>
            <td align="right" width="50%" class="formLabel">Click Browse to Upload Form:</td>
             ...
             <asp:RequiredFieldValidator ID="RequiredFieldValidator2" runat="server" ErrorMessage="* Please click Browse to upload your IRT form *" Display="Dynamic" CssClass="warningMessage" ControlToValidate="Filebox" /></td>
     </tr>
     <tr>
