...
<script runat="server">
    ...
    Protected Sub btnUploadForm_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        strSql = "p_PCAEmployeeUploadDMCSROBForm"
        cmd = New SqlCommand(strSql)
        cmd.CommandType = CommandType.StoredProcedure
        
        cmd.Parameters.AddWithValue("@ID", lblID.Text)
        cmd.Parameters.AddWithValue("@DMCS_ROB_Date", txtDMCS_ROB_Date.Text)
        ...
        If strFileNamePath.Length > 0 Then
            ...
            'strSaveLocation = "D:\DCS\secure\IMF_ED\x49g\pca\employees\dmcs_rob\" & strFileNameOnly
            strSaveLocation = lblUploadPath.Text & "pca\employees\dmcs_rob\" & strFileNameOnly
            Filebox.PostedFile.SaveAs(strSaveLocation)
            cmd.Parameters.Add("@DMCS_ROB_Attachment", SqlDbType.VarChar).Value = strFileNameOnly
        Else
            cmd.Parameters.Add("@DMCS_ROB_Attachment", SqlDbType.VarChar).Value = DBNull.Value
        End If
        ...
        lblUpdateStatus.Text = "Your DMCS ROB form has been received. Please click the Close Window button to return to the Employee Details screen."
    ...
    Sub ResetAcceptFormValue()
        ...
        cmd.CommandType = CommandType.StoredProcedure
        cmd.Parameters.AddWithValue("@ID", lblID.Text)
        cmd.Parameters.AddWithValue("@FormType", "DMCS_ROB")
...
<head runat="server">
    <title>Upload DMCS ROB Form</title>
...
<body>
    <form id="form1" runat="server">
     <fieldset>
    <legend class="fieldsetLegend">Upload DMCS Rules of Behavior (ROB) Form</legend><br />    
    <div align="left">
    <table>
    <tr>
            <td class ="formLabel" width="50%" >Date DMCS ROB Form Completed:</td>
            <td width="50%" ><asp:Textbox ID="txtDMCS_ROB_Date" runat="server"  /> (mm/dd/yyyy)<br />
            <asp:RequiredFieldValidator ID="RequiredFieldValidator1"
                    runat="server" ErrorMessage="* This field is required * " CssClass="warningMessage" Display="Dynamic" ControlToValidate="txtDMCS_ROB_Date" />
                    <asp:CompareValidator ID="dateValidator" runat="server" Type="Date" Operator="DataTypeCheck" ControlToValidate="txtDMCS_ROB_Date" ErrorMessage="* Please enter a valid date (mm/dd/yyyy) *" CssClass="warningMessage" Display="Dynamic" /></td>              
    </tr>        
    <tr>
            <td align="right" width="50%" class="formLabel">Click Browse to Upload DMCS ROB Form:</td>
             <td align="left" width="50%"><input id="Filebox" type="File" runat="server" size="35" class="button" /><br />
             <asp:RequiredFieldValidator ID="RequiredFieldValidator2" runat="server" ErrorMessage="* Please click Browse to upload your DMCS ROB form *" Display="Dynamic" CssClass="warningMessage" ControlToValidate="Filebox" /></td>
     </tr>
     <tr>
