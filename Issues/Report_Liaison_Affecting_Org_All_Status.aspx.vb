...
Partial Class Issues_Report_Liaison_Affecting_Org_All_Status
    ...
    Sub btnSearch_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        'Get all of the IssueStatus values
        ...
        If ddlIssueStatus.SelectedValue <> "" Then
            ...
            strSearchValue = Left(strSearchValue, (Len(strSearchValue) - 1))
            strSearchValue = Replace(strSearchValue, ",", ",")
            dsGridView.SelectParameters("IssueStatus").DefaultValue = strSearchValue
        End If

        'Get all of the FSA Group values
        'This one passes a comma-delimited string for @FSAGroup which is used in the split function
        If ddlFSAGroup.SelectedValue <> "" Then
            Dim strSearchValue As String = ""
            Dim li As ListItem
            For Each li In ddlFSAGroup.Items
                If li.Selected = True Then
                    strSearchValue = strSearchValue & li.Value & ","
                End If
            Next
            strSearchValue = Left(strSearchValue, (Len(strSearchValue) - 1))
            strSearchValue = Replace(strSearchValue, ",", ",")
            dsGridView.SelectParameters("FSAGroup").DefaultValue = strSearchValue
        End If
